...
$.get("https://randomuser.me/api/?results=20", function (data) {
	...
	friends.map((friend) => {
		...
		var nameBox = makeHTMLElement("div", "name-box", name);
		var username = `@${friend.login.username}`;
		var unBox = makeHTMLElement(
			"div",
			"user-name-box",
			username + " sent you a friend request."
		);
		var rBtnrow = makeHTMLElement("div", "request-btn-row", "", {
			"data-username": friend.login.username
		});

		var accept = makeHTMLElement(
			"button",
			"friend-request accept-request",
			"Accept",
			{ "data-username": friend.login.username }
		);
		var decline = makeHTMLElement(
			"button",
			"friend-request decline-request",
			"Decline",
			{ "data-username": friend.login.username }
		);
